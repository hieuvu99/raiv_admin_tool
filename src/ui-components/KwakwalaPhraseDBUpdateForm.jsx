...
export default function KwakwalaPhraseDBUpdateForm(props) {
  const {
    id: idProp,
    kwakwalaPhraseDB: kwakwalaPhraseDBModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = kwakwalaPhraseDBRecord
      ? { ...initialValues, ...kwakwalaPhraseDBRecord }
      : initialValues;
    setPhraseId(cleanValues.phraseId);
    setPhraseSetId(cleanValues.phraseSetId);
    setDescription(cleanValues.description);
    setOriginal(cleanValues.original);
    setTranslated(cleanValues.translated);
    setEnabled(cleanValues.enabled);
    setLanguageId(cleanValues.languageId);
    setExampleAudioPath(cleanValues.exampleAudioPath);
    setRecording1Start(cleanValues.recording1Start);
    setRecording1Stop(cleanValues.recording1Stop);
    setRecording2Start(cleanValues.recording2Start);
    setRecording2Stop(cleanValues.recording2Stop);
    setErrors({});
  };
  const [kwakwalaPhraseDBRecord, setKwakwalaPhraseDBRecord] = React.useState(
    kwakwalaPhraseDBModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(KwakwalaPhraseDB, idProp)
        : kwakwalaPhraseDBModelProp;
      setKwakwalaPhraseDBRecord(record);
    };
    queryData();
  }, [idProp, kwakwalaPhraseDBModelProp]);
  React.useEffect(resetStateValues, [kwakwalaPhraseDBRecord]);
  const validations = {
    phraseId: [],
    phraseSetId: [],
    description: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            KwakwalaPhraseDB.copyOf(kwakwalaPhraseDBRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "KwakwalaPhraseDBUpdateForm")}
      {...rest}
    >
      <TextField
        label="Phrase id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || kwakwalaPhraseDBModelProp)}
